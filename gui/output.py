...
class Output():
    ...
    def begin(self, total_time: [], approximate_roots_lists: [], index_dictionary: [], iterations_list: [],
              method_list: [], errors: []):
        self.iterations = [i + 1 for i in range(len(iterations_list))]
        self.iterations_num = len(iterations_list)
        root = tk.Tk()
        ...
        for i in range(len(approximate_roots_lists)):
            self.render(frame, total_time[i], approximate_roots_lists[i], index_dictionary, method_list[i], errors[i],
                        iterations_list)
