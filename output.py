...
class Output():
    ...
    def make_table(self,root,approximate_roots_lists,index_dictionary):
        frame = ScrollableFrame(root)
        keys_list = list(index_dictionary)

        for approximate_roots_list in approximate_roots_lists:
            for i in range(len(approximate_roots_list)):
                listBox = Listbox(frame.scrollable_frame, height=1, width=53, font=20)
                listBox.insert(END, f" {keys_list[i]} : {approximate_roots_list[i]}")
